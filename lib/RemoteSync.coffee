...
class RemoteSync
  ...
  createTransport: (host)->
    if host.transport is 'scp' or host.transport is 'sftp'
      ...
      throw new Error("[lua-remote-editor] invalid transport: " + host.transport + " in " + @configPath)
  ...
  diffFile: (localPath)->
    ...
    targetPath = path.join os.tmpDir(), "lua-remote-editor"
  ...
  diff: (localPath, targetPath) ->
    ...
    diffCmd = atom.config.get('lua-remote-editor.difftoolCommand')
    exec ?= require("child_process").exec
    exec "\"#{diffCmd}\" \"#{localPath}\" \"#{targetPath}\"", (err)->
      return if not err
      getLogger().error """Check [difftool Command] in your settings (lua-remote-editor).
       Command error: #{err}
...
module.exports =
  create: (projectPath)->
    configPath = path.join projectPath, atom.config.get("lua-remote-editor.configFileName")
